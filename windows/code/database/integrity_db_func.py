...
# Define path of database integrity
...
def create_integrity_db():
    try:
        ...
        with conn:
            ...
            # print(CREATE_DB_SUCCESS_MSG + "hash_file.table")
            ...
            sql_query = "CREATE TABLE IF NOT EXISTS hash_registry(" \
                        ...
                        + "last_change INTEGER)"
            cur.execute(sql_query)
            # print(CREATE_DB_SUCCESS_MSG + "hash_registry.table")
            ...
            cur.execute(sql_query)
            # print(CREATE_DB_SUCCESS_MSG + "sys_check_object.table")
            ...
            sql_query = "CREATE TABLE IF NOT EXISTS alert_integrity(" \
                        ...
                        + "path TEXT(260))"
            cur.execute(sql_query)
            # print(CREATE_DB_SUCCESS_MSG + "alert_integrity.table")
...
def insert_hash_record(type_object, path_object, hash_str):
    try:
        conn = get_connect_db(INTEGRITY_DB_PATH)
        with conn:
            cur = conn.cursor()
...
# Update hash record by id to database
def update_hash_string_by_id(type_object, id_object, hash_str):
    try:
        conn = get_connect_db(INTEGRITY_DB_PATH)
        with conn:
            cur = conn.cursor()
            if type_object == FILE_TYPE:
                cur.execute("UPDATE " + "hash_file "
                            "SET hash_str = ? WHERE id_file = ?", (hash_str, id_object))
            if type_object == REGISTRY_TYPE:
                cur.execute("UPDATE " + "hash_registry "
                            "SET hash_str = ? WHERE id_registry = ?", (hash_str, id_object))
                conn.commit()
            return SUCCESS_CODE
    except sqlite3.Error:
        print(QUERY_TABLE_DB_ERROR_MSG)
...
def insert_or_update_sys_check_object(path_object, type_object):
    try:
        ...
        with conn:
            cur = conn.cursor()
            # Search object exist in database
            cur.execute("SELECT * " +
